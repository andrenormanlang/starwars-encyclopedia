...
const SearchPeople = () => {
  ...
  const fetchAllPeople = async (searchPage = 0) => {
    setError(null);
    setLoading(true);
    setSearchResult(null);

    try {
      const res = await getAllPeople(searchPage);
      setSearchResult(res);
    } catch (err: any) {
  ...
  return (
    <>
      ...
      {!query && searchResult && (
        <p>
          There are a total of {searchResult.total} people in Star Wars episodes
          1 to 6. Use the force and do a search in the query above.
        </p>
      )}
